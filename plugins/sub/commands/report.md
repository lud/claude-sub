...
!`"${CLAUDE_PLUGIN_ROOT}/scripts/parent.sh"`
...
One `SendMessage` to the bare nickname. Cover what changed **since your last
report**, not the whole history:

- What was done, and the current state.
- Files changed, with paths.
- Decisions the user made in this pane that diverge from the briefing — the parent
  has no visibility into this conversation.
- What remains, or what you are blocked on.

Open by naming yourself and the task in one line: the parent may have started you
many turns ago and has never seen your pane. It sees only this message — no
transcript, no tool output, no plain text. Make it self-contained and report
outcomes faithfully.
