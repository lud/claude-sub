...
- **Do not send an acknowledgement.** The parent already knows you started — it
  started you. Go straight to the task.
- When the task is done, send the parent **one** `SendMessage` addressed to the
  bare nickname `{{PARENT}}`. Open it by naming yourself and the task in one line
  — the parent may have started you turns ago and never seen your pane. It cannot
  see your tool calls or your plain-text output, only what you put in that
  message. Make it self-contained: what you did and the outcome, files changed
  with paths, anything that blocked you, anything you decided differently from
  this briefing, and any wrong premise this briefing contained.
